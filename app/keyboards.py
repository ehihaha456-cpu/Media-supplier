...
def main_keyboard(settings=None):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⚙️ Settings", callback_data="settings"),
         InlineKeyboardButton("✏️ Editor", callback_data="editor")],
        [InlineKeyboardButton("👥 Users", callback_data="users"),
         InlineKeyboardButton("💬 Groups & Channels", callback_data="chats")],
...
def editor_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔘 Add Buttons", callback_data="add_button")],
...
def source_keyboard(chats):
    ...
    for c in chats[:20]:
        ...
        rows.append([InlineKeyboardButton(
            f"{icon} {title}",
            callback_data=f"set_source:{c['chat_id']}"
        )])
    rows += [
        [InlineKeyboardButton("➕ Set from group/channel command", callback_data="source_help")],
...
def chat_keyboard(chats):
    ...
    for c in chats[:30]:
        ...
        rows.append([InlineKeyboardButton(
            f"{icon} {title} · {state}",
            callback_data=f"toggle_chat:{c['chat_id']}"
        )])
